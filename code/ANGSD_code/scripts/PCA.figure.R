...
eigen.values$Percent <- eigen.values$values/sum(eigen.values$values)
...
write.csv(all_data.merged,"PCA.csv")
plots.bar <- all_data.merged  %>%
  ggplot(.,aes(x=PC1,PC2,color=Location,shape=Sex)) + 
  geom_point(size=3) +
  xlab(paste("Principal Component 1 (",round(100*eigen.values$Percent[1],2),"%)",sep="")) + ylab(paste("Principal Component 2 (",round(100*eigen.values$Percent[2],2),"%)",sep="")) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 60, hjust = 1),
        panel.grid.major = element_blank(),  # Remove major gridlines
    panel.grid.minor = element_blank() ) 
PCA1_2_plot.name <- paste0(out.dir,"/",name,"Figure.PC1_2.png")

ggsave(PCA1_2_plot.name ,plots.bar, width = 11, height= 6)
