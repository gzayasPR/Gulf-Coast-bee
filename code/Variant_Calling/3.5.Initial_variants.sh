...
#rm ${output_dir}/genome_intervals_0.bed
...
for chunk in ${output_dir}/genome_intervals_*.bed; do
    {
        ...
        sbatch --job-name="${name}_chunk" --output="${name}_chunk.out" --error="${name}_chunk.err" ../scripts/initial_joint.genotyping.sh $name $chunk_output_dir $chunk $gvcf_list ${ref_genome} ${output_dir}
