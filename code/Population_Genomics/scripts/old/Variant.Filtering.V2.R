...
results <- data.frame(
  ...
  maf_threshold = numeric(), min.MAC = numeric(), order_of_filtering = character(),
...
for (i in 1:nrow(parameters)) {
  ...
  param_set <- list(min.dp = param_row$min.dp, max.dp = param_row$max.dp,
                    ...
                    maf_threshold = param_row$maf_threshold, min.MAC = param_row$min.MAC, order_of_filtering = param_row$order_of_filtering)
  ...
  if (nrow(subset(results, min.dp == param_set$min.dp & max.dp == param_set$max.dp &
                  ...
                  maf_threshold == param_set$maf_threshold & min.MAC == param_set$min.MAC & order_of_filtering == param_set$order_of_filtering)) == 0) {
    ...
    # Calculate MAF or MAC for each locus based on the parameters
    if (!is.na(param_set$min.MAC)) {
      mac_values <- apply(gt_matrix, 1, calculate_mac)
      mac_filter <- which(as.numeric(mac_values) >= as.numeric(param_set$min.MAC))
      gt_matrix <- gt_matrix[mac_filter,]
    } else if (!is.na(param_set$maf_threshold)) {
    ...
    results <- rbind(results, data.frame(min.dp = param_set$min.dp, max.dp = param_set$max.dp, 
                                         min.missing.indiv = param_set$min.missing.indiv, min.missing.locus = param_set$min.missing.locus,
                                         maf_threshold = param_set$maf_threshold, min.MAC = param_set$min.MAC, order_of_filtering = param_set$order_of_filtering,
